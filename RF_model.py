...
# 训练数据存储文件夹
CSV_FILES = "datasets/Training_data"
...
# Save the model
def save_model(models_path, model_name, model):
    with open(os.path.join(models_path, f"{model_name}.pkl"), "wb") as file:
        pickle.dump(model, file)
    print(f'Model: {model_name}.pkl is saved to {models_path}')


# Load the model
def load_model(models_path, model_name):
    with open(os.path.join(models_path, f"{model_name}.pkl"), "rb") as file:
        model_loaded = pickle.load(file)
    print(f'Model: {model_name}.pkl is loaded!')
    return model_loaded


def train_RF(dataset, model_name):
    # 读取数据
    # csv_files = glob.glob("datasets/Training_data/*.csv")  # 调整路径
    csv_files = glob.glob(os.path.join(CSV_FILES, dataset))  # 调整路径
    ...
    # 定义需要移除的特征列
    drop_cols = ["business_id", "name", "address", "city", "state", "postal_code", "geometry", "latitude", "longitude",
                 "is_open"]
    ...
    # save_model(models_path=MODELS_PATH, model_name="rf_regressor_XGL", model=rf_regressor)
    save_model(models_path=MODELS_PATH, model_name=model_name, model=rf_regressor)

    return rf_regressor

if __name__ == '__main__':
    train_RF('XGL_results_20250223_091807.csv', 'rf_regressor_XGL')
    rf_regressor = load_model(MODELS_PATH,'rf_regressor_XGL')
    print("Model loaded!")